...
# Create RoleId and SecretID with secure permissions
ROLE_ID="$(vault read -field=role_id auth/approle/role/myapp/role-id)"
printf "%s" "$ROLE_ID" > /vault/file/role_id.txt
chmod 0600 /vault/file/role_id.txt
unset ROLE_ID

SECRET_ID="$(vault write -f -field=secret_id auth/approle/role/myapp/secret-id)"
printf "%s" "$SECRET_ID" > /vault/file/secret_id.txt
chmod 0600 /vault/file/secret_id.txt
unset SECRET_ID
...
if [ -n "$WRAP_TOKEN" ]; then
  printf "%s" "$WRAP_TOKEN" > /vault/file/wrapping_token.txt
  chmod 0600 /vault/file/wrapping_token.txt
fi
unset WRAP_TOKEN WRAP_JSON

touch /vault/file/.vault_configured

# Cleanup: remove root token from memory
unset VAULT_TOKEN
