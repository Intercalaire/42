...
# Secure token handling: write to file with restricted permissions only
printf "%s" "$ROOT_TOKEN" > /vault/file/root.token.tmp
chmod 0600 /vault/file/root.token.tmp
unset ROOT_TOKEN
...
# Load token for config script only
export VAULT_TOKEN="$(cat /vault/file/root.token.tmp)"
